...
PgnLib::PgnValidator::PgnValidator(PgnMoveMaps *data) : mValidateData(std::move(data))
{
    PgnValidate();
}

PgnLib::PgnValidator::~PgnValidator()
{
    mValidateData.reset();
}
...
void PgnLib::PgnValidator::PgnValidate() const
{
    ...
    {
        std::cout << "****** Validating Game: " << gameCount << " ******" << std::endl;

        for (const auto &ity : itx)
        {
            std::cout << " Move --> [ "
                      << ity.first << ", " << Validate(ity.second.first) << " : "
...
Misc::Validity PgnLib::PgnValidator::Validate(const std::string &pgn) const
{
    ...
    std::regex game_notation("([RNBKQP]?)([a-h]|[1-8]?)([x]?)([a-h])"
                             "([1-8])([+|#]?)|(O-O-O)|(O-O)");

    return (std::regex_match(pgn, mSmatch, game_notation)) ? Misc::Valid : Misc::Invalid;
}
